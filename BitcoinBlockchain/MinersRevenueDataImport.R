...
btcMinersRevenueNameString <- c("C:/Projects/BitcoinDataTable/btcMinersRevenueDataSeries.Rdata");

full.file.names <- NULL;
save.file.names <- NULL;
btcMinersRevenue <- NULL; #declare the merged data series object as a NULL to assigned as data.table later on in the code
tempbtcMinersRevenueTablesList <- NULL; #declare the merged data table list object as a NULL for use with the rbindlist(a,b) call 
...
for (i in SOURCE.dir){
  setwd(i);
  tmp <- list.files(pattern="miners-revenue.txt");
...
source.file.process <- function(x){
  ...
  colnames(clean.data) <- c("date.time","TotalRevenue");
...
for (i in 1:length(full.file.names)){
  ...
  #btcVolume datatable. 
  if(is.null(btcMinersRevenue)){
    btcMinersRevenue  <- source.file.process(full.file.names[i]);
  } else{
    tempbtcVolumeTablesList <- list(btcMinersRevenue,source.file.process(full.file.names[i]));
    btcMinersRevenue  <- rbindlist(tempbtcMinersRevenueTablesList);
  }
  ...
  if(i == length(save.file.names)){
    save(btcMinersRevenue,file=btcMinersRevenueNameString);
  }
  gc();
}
