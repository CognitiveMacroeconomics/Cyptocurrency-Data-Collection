...
btcTotalInCirculationNameString <- c("C:/Projects/BitcoinDataTable/btcTotalInCirculationDataSeries.Rdata");

full.file.names <- NULL;
save.file.names <- NULL;
btcTotalInCirculation <- NULL; #declare the merged data series object as a NULL to assigned as data.table later on in the code
tempbtcTotalInCirculationTablesList <- NULL; #declare the merged data table list object as a NULL for use with the rbindlist(a,b) call 
...
for (i in SOURCE.dir){
  setwd(i);
  tmp <- list.files(pattern="total-bitcoins.txt");
...
source.file.process <- function(x){
  ...
  colnames(clean.data) <- c("date.time","TotalBTC");
...
for (i in 1:length(full.file.names)){
  ...
  #btcVolume datatable. 
  if(is.null(btcTotalInCirculation)){
    btcTotalInCirculation  <- source.file.process(full.file.names[i]);
  } else{
    tempbtcVolumeTablesList <- list(btcTotalInCirculation,source.file.process(full.file.names[i]));
    btcTotalInCirculation  <- rbindlist(tempbtcTotalInCirculationTablesList);
  }
  ...
  if(i == length(save.file.names)){
    save(btcTotalInCirculation,file=btcTotalInCirculationNameString);
  }
  gc();
}
