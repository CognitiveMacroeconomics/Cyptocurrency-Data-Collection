...
btcOutputVolumeNameString <- c("C:/Projects/BitcoinDataTable/btcOutputVolumeDataSeries.Rdata");

full.file.names <- NULL;
save.file.names <- NULL;
btcOutputVolume <- NULL; #declare the merged data series object as a NULL to assigned as data.table later on in the code
tempbtcOutputVolumeTablesList <- NULL; #declare the merged data table list object as a NULL for use with the rbindlist(a,b) call 
...
for (i in SOURCE.dir){
  setwd(i);
  tmp <- list.files(pattern="output-volume.txt");
...
source.file.process <- function(x){
  ...
  colnames(clean.data) <- c("date.time","OutputVolume");
...
for (i in 1:length(full.file.names)){
  ...
  #btcVolume datatable. 
  if(is.null(btcOutputVolume)){
    btcOutputVolume  <- source.file.process(full.file.names[i]);
  } else{
    tempbtcVolumeTablesList <- list(btcOutputVolume,source.file.process(full.file.names[i]));
    btcOutputVolume  <- rbindlist(tempbtcOutputVolumeTablesList);
  }
  ...
  if(i == length(save.file.names)){
    save(btcOutputVolume,file=btcOutputVolumeNameString);
  }
  gc();
}
