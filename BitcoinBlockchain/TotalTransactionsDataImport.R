...
btcTradeVolumeNameString <- c("C:/Projects/BitcoinDataTable/btcTradeVolumeDataSeries.Rdata");

full.file.names <- NULL;
save.file.names <- NULL;
btcTradeVolume <- NULL; #declare the merged data series object as a NULL to assigned as data.table later on in the code
tempbtcTradeVolumeTablesList <- NULL; #declare the merged data table list object as a NULL for use with the rbindlist(a,b) call 
...
for (i in SOURCE.dir){
  setwd(i);
  tmp <- list.files(pattern="trade-volume.txt");
...
source.file.process <- function(x){
  ...
  colnames(clean.data) <- c("date.time","TotalTradeVolume");
...
for (i in 1:length(full.file.names)){
  ...
  #btcVolume datatable. 
  if(is.null(btcTradeVolume)){
    btcTradeVolume  <- source.file.process(full.file.names[i]);
  } else{
    tempbtcVolumeTablesList <- list(btcTradeVolume,source.file.process(full.file.names[i]));
    btcTradeVolume  <- rbindlist(tempbtcTradeVolumeTablesList);
  }
  ...
  if(i == length(save.file.names)){
    save(btcTradeVolume,file=btcTradeVolumeNameString);
  }
  gc();
}
