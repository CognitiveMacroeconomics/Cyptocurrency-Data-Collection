...
btcDifficultyNameString <- c("C:/Projects/BitcoinDataTable/btcDifficultyDataSeries.Rdata");

full.file.names <- NULL;
save.file.names <- NULL;
btcDifficulty <- NULL; #declare the merged data series object as a NULL to assigned as data.table later on in the code
tempbtcDifficultyTablesList <- NULL; #declare the merged data table list object as a NULL for use with the rbindlist(a,b) call 
...
for (i in SOURCE.dir){
  setwd(i);
  tmp <- list.files(pattern="difficulty.txt");
...
source.file.process <- function(x){
  ...
  colnames(clean.data) <- c("date.time","difficulty");
...
for (i in 1:length(full.file.names)){
  ...
  #btcVolume datatable. 
  if(is.null(btcDifficulty)){
    btcDifficulty  <- source.file.process(full.file.names[i]);
  } else{
    tempbtcVolumeTablesList <- list(btcDifficulty,source.file.process(full.file.names[i]));
    btcDifficulty  <- rbindlist(tempbtcDifficultyTablesList);
  }
  ...
  if(i == length(save.file.names)){
    save(btcDifficulty,file=btcDifficultyNameString);
  }
  gc();
}
